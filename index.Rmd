...
Note: After running the train() function from caret it turned out that bootstrapping was used even though trainControl(method="repeatedcv", number=10, repeats = 10) or trainControl(method="cv", number=10) was set i.e. caret ignored the trainControl() setting, therefore the K-fold CV is done manually outside caret as follows.

```{r}
folds <- createFolds(training$classe, k = 5)
```

Thus there are five pairs of training and validation sets i.e. the first training set is available as training[-folds$Fold1,] and the corresponding validation set is  training[folds$Fold1,] and so on.
...
It turns out that the rf model performs ("in sample") nearly identical on the five (K-fold) training sets.
...
However the real tests are how well the trained models perform on the five validation sets and the (single) testing set. Here it also turns that they all give near identical results with very low bias (i.e. precise results) and low variance (i.e. low variance between different validation sets).

Prediction and accuracy calculation on a validation set is done as, and the accuracy of the first is 0.9997452, and the other trained models give near identical results:
...
Prediction on the testing set is done on of the five trained models (they all give the same results) as: 
